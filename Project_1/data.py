...
class Data:
    ...
    def limit_samples(self, start_row, end_row):
        ...
        counter = 0
        for i in self.data:
            if(counter >= start_row and counter < end_row):
                rowdata = []
                for datum in i:
                    rowdata.append(i)
    ...
    def select_data(self, headers, rows=[]):
        ...
        colSkips = []
        for i in headers:
            colSkips.append(self.get_header_indices[i])

        for i in self.data:
            rowData = []
            if(rowCounter not in rows):
                colCounter = 0
                for datum in i:
                    if(colCounter not in colSkips):
                        rowData.append(datum)
                    colCounter+=1
            rowCounter+=1

        return np.arr(subset)
